...
export default function ReviewCommentsModal({ isOpen, onClose, review }) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black/80 flex items-center justify-center p-4 backdrop-blur-2xl animate-fade-in">
      <div 
        className="w-full max-w-xl max-h-[85vh] flex flex-col rounded-3xl overflow-hidden border border-white/14 bg-[#121218]/95 text-slate-100 shadow-[0_25px_70px_rgba(0,0,0,0.95),0_0_30px_rgba(229,9,20,0.12)]"
      ...
      >
        ...
        {/* Review snippet card */}
        <div className="p-4.5 bg-white/4 border-b border-white/8 shrink-0 space-y-2.5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Avatar username={review.username} url={review.avatar_url} className="w-6 h-6 ring-1 ring-white/15" />
              <span className="font-sans font-bold text-xs text-slate-200">@{review.username}</span>
            </div>
            <RatingBadge rating={review.rating} size="xs" />
          </div>
          <p className="text-xs font-sans text-slate-200 line-clamp-3 italic bg-black/40 p-3.5 rounded-2xl border border-white/6 leading-relaxed text-left">
            "{review.review_text}"
          </p>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 bg-[#0a0a0f]/80">
          {loading ? (
            ...
            comments.map((c) => (
              <div key={c.id} className="p-3.5 bg-white/5 rounded-2xl border border-white/8 text-xs space-y-1.5 text-left">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Avatar username={c.username} url={c.avatar_url} className="w-5 h-5 ring-1 ring-white/10" />
                    <span className="font-sans font-bold text-[#ff4d5a]">@{c.username}</span>
        ...
        {/* Input box */}
        <form onSubmit={handleAddComment} className="p-3.5 bg-white/5 border-t border-white/8 flex gap-2 shrink-0">
          <input
            type="text"
            ...
            className="flex-1 bg-black/60 border border-white/12 rounded-2xl px-4 py-2.5 text-xs font-sans text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-[#e50914]"
          />
          <button
            type="submit"
            disabled={!user || submitting || !newComment.trim()}
            className="btn-primary px-4 py-2 text-xs flex items-center gap-1.5 disabled:opacity-40 font-display font-bold uppercase tracking-wider rounded-2xl shadow-md cursor-pointer"
          >
